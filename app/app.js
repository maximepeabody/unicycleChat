...
.config(['FacebookProvider', function(FacebookProvider) {
  ...
  .controller('CategoryCtrl', [
    ...
    'CurrentGroup',
    'Facebook',
    function($scope, CurrentGroup, Facebook) {
      $scope.currentGroup = CurrentGroup;
      $scope.groups = [{
        id: '1170550659625884',
        name: "Urban Unicycle chat"
      }, {
        id: '115835695144753',
        name: "Unicycle Chat"
      }, {
      ...
      $scope.addGroup = function(query) {
        $scope.query =""
        Facebook.api('/search?q=' + query + '&type=group', function(response) {
          if (response.error) {
...
.controller('HeaderCtrl', [
  ...
  function($scope, Query) {
    ...
    $scope.setIn = function(s) {
      $scope.search.in = s
    };
    $scope.setFilter = function(s) {
      $scope.search.filter = s
    };
    $scope.setOrder = function(s) {
      $scope.search.order = s
    };
  }
])
...
.controller('MainCtrl', [
  ...
  function($scope, $timeout, Facebook, Query, CurrentGroup, $sce) {
    ...
    $scope.loadFeed = function(id) {
      Facebook.api('/' + id + '/feed?fields=from,message,link,picture,source,full_picture,likes,type&limit(' + feedLimit +')', function(response) {
        console.log(response);
        if(response.data.length>0) {
          $scope.$apply(function() {
            ...
            $scope.previousPage = $scope.feed.paging.previous;
          });
        }
      });
    };
    ...
    $scope.getNextPage = function() {
      Facebook.api($scope.nextPage, function(response){
        if(response.data.length>0) {
          $scope.$apply(function() {
            $scope.feed = response;
            $scope.loadFeedProfilePictures($scope.feed);
    ...
    $scope.getPreviousPage = function() {
      Facebook.api($scope.previousPage, function(response){
        if(response.data.length>0) {
          $scope.$apply(function() {
            $scope.feed = response;
            $scope.loadFeedProfilePictures($scope.feed);
